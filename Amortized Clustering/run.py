...
parser.add_argument('--net', type=str, default='picaso_model')
...
if args.net == 'picaso_model':
    net = PicasoModel(D, K, dim_output).cuda()
elif args.net == 'set_transformer':
...
def plot():
    ...
    X, labels_r, pi_r, params_r = mog.sample(B, np.random.randint(N_min, N_max), K, return_gt=True)
    ll_r = mog.log_prob(X, pi_r, params_r).item()
    print(ll_r)
    pi, params = mvn.parse(net(X))
    ll, labels = mog.log_prob(X, pi, params, return_labels=True)
    print(ll)
    fig, axes = plt.subplots(2, B//2, figsize=(7*B//5,6))
